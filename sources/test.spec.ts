...
describe("contract", () => {
    ...
    beforeAll(async () => {
        ...
        const deploy_result = await collection.send(deployer.getSender(), { value: toNano(1) }, "MintFree");
    ...
    it("Test Mint Record in detail", async () => {
        const deploy_result = await collection.send(deployer.getSender(), { value: toNano(1) }, "MintFree"); // Send Mint Transaction
        printTransactionFees(deploy_result.transactions);
        prettyLogTransactions(deploy_result.transactions);
    });

    it("should deploy correctly", async () => {
        await collection.send(deployer.getSender(), { value: toNano(2) }, "MintFree");
